...
**Documented:** negative prompts accept unwanted details, changing model/aspect ratio/style is the next diagnostic path, and Canvas provides mask/erase editing for a selected area.
...
In the negative-prompt input exposed by the current interface, enter concise unwanted concepts as comma-separated terms. Prefer:

```text
extra fingers, duplicate limbs, background text
```

Avoid sentences built around “no” or “without,” such as `no extra fingers` or `portrait without text`. The field already supplies the negative role; direct terms keep the worksheet easy to inspect.
...
| Observed issue | First negative terms to try | What stays fixed | Next single variable if it persists |
|---|---|---|---|
...
Follow the source order while changing one field at a time:
...
4. **Style:** return to the recorded aspect ratio and change only the style control, when that selected model exposes one.
...
| 4 | same as step 1 | baseline | baseline | one alternative | none | Style is the only changed field |
