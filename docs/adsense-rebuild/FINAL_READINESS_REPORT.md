...
| Ads | Placement wrappers, scripts/slots/placeholders, and reserved empty inventory existed. | Ads are disabled; only inert centralized boundaries remain and no advertising request or placeholder appears. The existing public ads.txt seller declaration remains but does not enable ads and requires account verification. |
